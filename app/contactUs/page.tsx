      ...
      <div className="flex flex-col p-5 md:px-18 gap-8 md:gap-16.25 tracking-wider leading-loose">
        <section className="flex flex-col md:flex-row gap-8">
          <div className="flex flex-col gap-3 w-full md:w-1/2 md:gap-6 p-3">
            ...
            <h1 className="text-gray-800 tracking-wider font-semibold text-xl md:text-3xl leading-10">
            ...
            <div className="flex flex-col gap-3.5">
              ...
              <ul className="list-disc gap-2 text-[14px] md:text-[16px]">
                {interestsData.map((item, index) => (
                  <li key={index} className="md:text-slate-700">
                    {item}
                  </li>
                ))}
              </ul>
          ...
          <div className="flex h-auto md:w-1/2 shadow-xl  rounded-t-xl border-t-4 border-t-primaryGreen">
            {' '}
            <InquiryFormComponent formName="Send us an Inquiry" />
          </div>
        </section>
        <div ref={ref} className="flex relative h-175 md:h-120 mb-10 ">
          <Image
            src={backgroundImage}
          ...
          />
          <div className="flex h-full absolute inset-0 bg-black/50  flex-col  md:flex-row-reverse items-center justify-center text-white p-4 tracking-wider leading-loose gap-7 md:gap-18 w-full">
            <div className="flex flex-col w-full md:w-[40%] md:gap-4 md:-mr-8">
              {contactData.map((item, index) => (
                <div
                  key={index}
                  className={`flex w-full max-w-md  gap-3 p-3 md:gap-4 items-center transform transition-transform duration-1000 ease-linear ${
                    inView
                      ? '-translate-x-10 opacity-100'
                      : 'translate-x-0 opacity-0'
                  }`}
                >
                  <span className="flex  h-15 w-15 md:w-18 md:h-18 bg-primaryGreen justify-center items-center">
            ...
            <div className="w-full md:w-[60%] h-full px-2 mb-5 md:-mt-12">
              <iframe
